...
def Main():
    ...
    #KEYWORDS TO FIND
    persistentSearchCount = {"python": 0, "javascript": 0, "c++": 0, "sql": 0, "flutter": 0}
...
def AddToCSV(data: dict, keyString: dict, pages):
    data.update({"Checked Pages": pages})
    data.update({"Run Time": datetime.today().strftime("%d/%m/%Y|%H:%M:%S")})
